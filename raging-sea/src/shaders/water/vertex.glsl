...
uniform float uniformSmallWavesElevation;
uniform float uniformSmallWavesFrequency;
uniform float uniformSmallWavesSpeed;
uniform float uniformSmallWavesIterations;
...
void main() {
    ...
    // Add Perlin Noise at different frequencies.
    for (float i = 1.0; i <= uniformSmallWavesIterations; i ++) {
        elevation -= abs(cnoise(vec3(modelPosition.xz * uniformSmallWavesFrequency * i, uniformTime * uniformSmallWavesSpeed)) * uniformSmallWavesSpeed / i);
    }
